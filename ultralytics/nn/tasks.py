...
class BaseModel(nn.Module):
    ...
    def _predict_once(self, x, profile=False, visualize=False, embed=None):
        ...
        if isinstance(x,tuple):
            img_flag = torch.tensor(x)
            if x.is_cuda:
                img_flag = img_flag.cuda()
        else:
            img_flag = x
        for m in self.model:
            if isinstance(m, getir):
                x = x_ir
            if isinstance(m, getvi):
                if img_flag.shape[0] == 1:
                    x_ir = x
                else:
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        if m in {
            ...
            if m in {
                ...
                n = 1
            if m in {C3k2_irat,C3k2_irlb,C3k2} and scale in "mlx":  # for M/L/X sizes
                args[3] = True
        elif m is AIFI:
            args = [ch[f], *args]
            ...
            c1 = ch[f]
            args = [c1, c2, *args[1:]]
        elif m in {CBFuse, feature_fusion,AT_fuse}:
            c2 = ch[f[-1]]
        elif m is DenseFuse_net :
            c2 = args[0]
        elif m is ConcatFusion:
            c2 = ch[f[-1]]
            args = [ch[f[-2]], c2]
        # elif m is AttentionFusionModule:
        #     c2 = ch[f[1]]
        #     c1 = args[0]
        else:
            c2 = ch[f]
